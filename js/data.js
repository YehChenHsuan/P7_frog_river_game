/**
 * ALICE ESL Phonics P7 - 萌蛙過河單字大冒險題庫核心資料 (data.js)
 * 課本學習範圍：Page 04 - 12
...
const VOCAB_MAP = new Map(P7_VOCABULARY.map(v => [v.id, v]));
...
class QuestionGenerator {
  ...
  static generateListenQuestion() {
    const target = P7_VOCABULARY[Math.floor(Math.random() * P7_VOCABULARY.length)];
    const others = P7_VOCABULARY.filter(v => v.id !== target.id);
  ...
  static generatePhonicsQuestion() {
    ...
    const target = validTargets.length > 0 ? validTargets[Math.floor(Math.random() * validTargets.length)] : P7_VOCABULARY[0];
    
    // 干擾項
    const distractors = P7_VOCABULARY.filter(v => !targetWordIds.includes(v.id));
  ...
  static generateActionQuestion() {
    ...
    const target = VOCAB_MAP.get(qData.correct) || {
      ...
      word: qData.correct,
      zh: qData.correct,
      image: `P7_flashcards_images/P7_${qData.correct}.webp`,
      audioEn: `P7_flashcards_audios/P7_${qData.correct}.mp3`,
      audioZh: `P7_flashcards_audios/P7_${qData.correct}_zh.mp3`
    };
    ...
    const optionItems = qData.options.map(id => VOCAB_MAP.get(id) || {
      ...
      word: id,
      zh: id,
      image: `P7_flashcards_images/P7_${id}.webp`,
      audioEn: `P7_flashcards_audios/P7_${id}.mp3`,
      audioZh: `P7_flashcards_audios/P7_${id}_zh.mp3`
    });
